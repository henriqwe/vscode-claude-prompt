import { TurnRecord } from './runHistory'
...
export function renderConversationHtml(state: RenderState): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    * { box-sizing: border-box; }
    ...
    <div class="container">
      <header>
        <span class="status"><span id="dot" class="dot ${initialDot}"></span><span id="status">${state.running ? 'Thinking' : 'Done'}</span></span>
      ...
      <div id="output">${state.running ? '<div class="empty" id="placeholder">Waiting for response…</div>' : ''}${isReplay ? renderReplayStructure(state.replayTurns!) : ''}</div>
